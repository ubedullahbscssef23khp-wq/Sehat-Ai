# Phase 10 API Runtime Test

# Section 3.1: Create Session (English)
Write-Host "=== Section 3.2: Create Session (English) ===" -ForegroundColor Cyan
$payload = @{ preferred_language = 'EN' } | ConvertTo-Json
$response = Invoke-WebRequest -Uri http://127.0.0.1:8000/sessions -Method Post -UseBasicParsing -ContentType 'application/json' -Body $payload
...
$messagePayload = @{ text = "I have a headache and fever for the past 2 days. The headache is throbbing and gets worse with light." } | ConvertTo-Json
$msgResponse = Invoke-WebRequest -Uri "http://127.0.0.1:8000/sessions/$sessionId/messages" -Method Post -UseBasicParsing -ContentType 'application/json' -Body $messagePayload
